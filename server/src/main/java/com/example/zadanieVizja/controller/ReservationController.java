...
public class ReservationController {
    ...
    public ResponseEntity<ReservationResponse> assign(@PathVariable("id") Long id, Authentication auth) {
        String username = auth.getName();
        return ResponseEntity.ok(reservationService.assignToEmployee(username, id));
    }

    @GetMapping("/admin/assigned")
    public ResponseEntity<List<ReservationResponse>> listMyAssignedReservations(Authentication auth) {
        String username = auth.getName();
        return ResponseEntity.ok(reservationService.listAssignedToEmployee(username));
    }
